...
const HomeBook = ({ book }) => {
  const { name, coverImage, genre, author, tags, ratings } = book;
  return (
    <div>
      <div className="card bg-base-100 w-72 lg:w-96 h-[26rem] shadow-xl">
        ...
        <div className="card-body p-6 text-left">
          <ul className="flex gap-1 mb-2">
            {tags.slice(1, 3).map((element, i) => (
              <li key={i}>
                <button className="tags lg:hidden">{element.slice(0,9)}</button>
